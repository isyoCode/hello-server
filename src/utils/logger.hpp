...
template <class T>
inline std::unique_ptr<T> LoggerSingleton<T>::data_{nullptr};
template <class T>
inline std::mutex LoggerSingleton<T>::mtx;
template <class T>
T* LoggerSingleton<T>::getInstance() {
  if (data_ == nullptr) {
...
*/
template <class T>
class LoggerCirculQueen {
 public:
  LoggerCirculQueen() = default;
  /*留个空位作为 空/满的标识位置 */
  explicit LoggerCirculQueen(size_t maxSize)
      : _iMaxItem(maxSize + 1), _vQueen(_iMaxItem) {}
  LoggerCirculQueen(const LoggerCirculQueen&) = default;
  LoggerCirculQueen& operator=(const LoggerCirculQueen&) = default;
  LoggerCirculQueen(LoggerCirculQueen&& other) { rvalueCtor(std::move(other)); }
  LoggerCirculQueen& operator=(LoggerCirculQueen&& other) {
 ...
 private:
  void rvalueCtor(LoggerCirculQueen&& other) {
...
class BufferQueen {
 public:
  BufferQueen() : _dataQueen(LoggerCirculQueen<T>(_iBufferSize)) {}
  explicit BufferQueen(size_t buffer_size)
      : _dataQueen(LoggerCirculQueen<T>(buffer_size)) {}
 ...
 private:
  ...
  LoggerCirculQueen<T> _dataQueen;
};
...
class Logger : public LoggerSingleton<Logger> {
 public:
  Logger() : _logcof() {
    try {
      initiallize();
 ...
 public:
  template <class T>
  void trace(T&& str) {
    log(LOGLEVEL::TRACE, std::forward<T>(str));
  }
  template <class T>
  void info(T&& str) {
    log(LOGLEVEL::INFO, std::forward<T>(str));
  }
  template <class T>
  void debug(T&& str) {
    log(LOGLEVEL::DEBUG, std::forward<T>(str));
  }
  template <class T>
  void error(T&& str) {
    log(LOGLEVEL::ERROR, std::forward<T>(str));
  }
  template <class T>
  void warn(T&& str) {
    log(LOGLEVEL::WARNING, std::forward<T>(str));
  }
  template <class T>
  void fatal(T&& str) {
    log(LOGLEVEL::FATAL, std::forward<T>(str));
  }

 private:
  void log(LOGLEVEL level, std::string&& str) {
    _buffer.enqueen(Message{level, std::move(str), LocationInfo()});
  }
  void log(LOGLEVEL level, const std::string& str) {
    _buffer.enqueen(Message{level, str, LocationInfo()});
  }
  ...
  void rotateFile() {
    ...
    if (filesize < (int)_logcof._fileMaxSize) return;
    _logout.close();
 ...
 private:
  struct logConf {
    size_t _fileMaxSize;
    size_t _fileNum;
    size_t _fileBufferSize;     // 文件缓冲区大小
    size_t _consoleBufferSize;  // 控制台缓冲区大小
    bool _isStop;
    bool _isColor;
    bool _isConsle;
    bool _isWritefile;
    ...
    logConf()
        : _fileMaxSize(1024 * 1024 * 128),
          _fileNum(10),
          _fileBufferSize(1 * 1024),     // 默认1KB（开发环境）
          _consoleBufferSize(1 * 1024),  // 默认1KB
          _isStop(false),
          _isColor(true),
          _isConsle(true),
          _isWritefile(true),
 ...
 public:
  Logger& setLogDirName(std::string logDirName) {
    ...
    createlogDir();
    _logout.open(getLognName().append(".log"), std::ios::app);
    return *this;
  }
  ...
  // 设置缓冲区大小（需要在initiallize之前调用才有效）
  Logger& setBufferSizes(size_t fileBufferSize, size_t consoleBufferSize) {
    _logcof._fileBufferSize = fileBufferSize;
    _logcof._consoleBufferSize = consoleBufferSize;
    return *this;
  }
 ...
 private:
  ...
  void initiallize() {
    ...
    // 不要在初始化时关闭控制台和轮转，使用默认配置值
    // setConsle(false).setRotate(false);  //
    // 注释掉这行，保留默认的isConsle=true

    // 使用配置的缓冲区大小（通过setBufferSizes设置，或使用默认值）
    _fileCurrentBufferSize = _logcof._fileBufferSize;
    _consoleCurrentBufferSize = _logcof._consoleBufferSize;
...
#define LOGI(Msg) Logger::getInstance()->info(Msg);
#define LOGD(Msg) Logger::getInstance()->debug(Msg);
#define LOGW(Msg) Logger::getInstance()->warn(Msg);
#define LOGE(Msg) Logger::getInstance()->error(Msg);
#define LOGF(Msg) Logger::getInstance()->fatal(Msg);
#define LOGT(Msg) Logger::getInstance()->trace(Msg);
